...
switch_status_t switch_nhop_member_handle_init(switch_device_t device) {
  ...
  if (status != SWITCH_STATUS_SUCCESS) {
     VLOG_ERR("nhop member handle init Failed for device %d: %s\n",
                     device,
...
switch_status_t switch_nhop_init(switch_device_t device) {
  ...
  if (status != SWITCH_STATUS_SUCCESS) {
    VLOG_ERR("NHOP group init Failed for device %d\n",
                     device);
    return status;
  }
  ...
  if (status != SWITCH_STATUS_SUCCESS) {
    VLOG_ERR("nhop member init Failed for device %d: %s\n",
                     device,
                     switch_error_to_string(status));
    return status;
  }
...
switch_status_t switch_nhop_free(switch_device_t device) {
  ...
  if (status != SWITCH_STATUS_SUCCESS) {
    VLOG_ERR("NHOP group free Failed for device %d\n",
                     device);
  }
...
switch_status_t switch_api_nhop_group_create(const switch_device_t device,
  ...
  *nhop_group_handle = handle;
  ...
  VLOG_INFO("nhop group handle created on device %d nhop group handle 0x%lx\n",
                   device,
                   *nhop_group_handle);
...
switch_status_t switch_api_nhop_member_add (
    const switch_device_t device,
  ...
  for (index = 0; index < num_nhops; index++) {
    ...
    if (status != SWITCH_STATUS_SUCCESS) {
        VLOG_ERR("switch_pd_handle_member: Failed to create platform "
...
switch_status_t switch_api_nhop_group_delete(const switch_device_t device,
                                       const switch_handle_t nhop_group_handle) {
  switch_uint32_t index = 0;
  ...
  if (num_nhops) {
    ...
    status = switch_api_nhop_member_delete(
  ...
  VLOG_DBG("nhop_group handle deleted on device %d nhop_group handle 0x%lx\n",
                   device,
                   nhop_group_handle);
...
switch_status_t switch_nhop_member_get_from_nhop(
  ...
  if (!nhop_member) {
    status = SWITCH_STATUS_INVALID_PARAMETER;
    VLOG_ERR("Failed to get nhop member on device %d: %s",
                     device,
                     switch_error_to_string(status));
    return status;
  }
  ...
  if (!SWITCH_NHOP_GROUP_HANDLE(nhop_group_handle)) {
    status = SWITCH_STATUS_INVALID_PARAMETER;
    VLOG_ERR("Failed to get nhop_group member on device %d: %s",
                     device,
                     switch_error_to_string(status));
    return status;
  }
...
switch_status_t switch_api_nhop_member_delete(
    const switch_device_t device,
    const switch_handle_t nhop_group_handle,
    const switch_uint32_t num_nhops,
    const switch_handle_t *nhop_handles) {
...
switch_status_t switch_api_nhop_members_delete (
  ...
  VLOG_INFO("nhop members deleted on device %d nhop_group handle 0x%lx\n",
                   device,
                   nhop_group_handle);
