...
class LNT_FRR_without_ECMP_Netperf(BaseTest):
    ...
    def runTest(self):
        ...
        for vm, port in zip(self.config_data['vm'], self.config_data['port']):
           globals()["conn"+str(vm_id+1)] = connectionManager("127.0.0.1", f"655{vm_id}", vm['vm_username'],\
                                            vm['vm_password'],timeout=self.config_data['netperf']['testlen']+5)
        ...
        if not test_utils.vtysh_config_frr_router_interface(self.config_data['ecmp']['local_ports'][0],
                ip= self.config_data['ecmp']['local_ports_ip'][0] ):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"Failed to config frr inteface {self.config_data['ecmp']['local_ports_ip'][0]} on local")
            
        #configure BGP attributes on frr                                             
        if not test_utils.vtysh_config_frr_bgp_attr(self.config_data['bgp']['as_number'],
                    neighbor=self.config_data['ecmp']['remote_ports_ip'][0].split("/")[0], addr_family= self.config_data['bgp']['addr_family'], 
        ...
        print (f"Add device {self.config_data['vxlan']['tep_intf']} on remote {self.config_data['client_hostname']}")
        ...
        if not test_utils.vtysh_config_frr_router_interface(self.config_data['vxlan']['tep_intf'],
                ip= self.config_data['vxlan']['tep_ip'][1],hostname=self.config_data['client_hostname'],
                    username=self.config_data['client_username'], password=self.config_data['client_password']):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL: Failed to config frr inteface {self.config_data['vxlan']['tep_intf']} on {self.config_data['client_hostname']}")
            
        #configure TAP on remote frr   
        if not test_utils.vtysh_config_frr_router_interface(self.config_data['remote_port'][0],
                ip= self.config_data['ecmp']['remote_ports_ip'][0], hostname=self.config_data['client_hostname'],
                    username=self.config_data['client_username'],password=self.config_data['client_password']):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"Failed to config frr inteface {self.config_data['ecmp']['remote_ports_ip'][0]} on {self.config_data['client_hostname']}")
        ...
        print("\nSleep before sending ping and netperf traffic")
        time.sleep(15)
        ...
        for i in range(len(self.conn_obj_list)):
            ...
            for m in range(len(self.config_data['vm'][i]['remote_ip'])):
                ...
                for testname in self.config_data['netperf']['testname']:  
                    ...
                    if m==0: print ("Verify local VM netperf traffic without ecmp path")
                    
                    if m != 0: # netperf traffic is sent acroos emcp link
                        print(f"Send netperf from VM{i} to {ip} on other VMs with ECMP path")
                        print (f"Record port {self.config_data['port'][i]['interface']} counter before sending traffic on vm{i}")
                        vm_int_count_before = test_utils.get_vm_interface_counter(self.conn_obj_list[i],self.config_data['port'][i]['interface'])
                    ...
                    if m!=0:       
                        print (f"Record port {self.config_data['port'][i]['interface']} counter after sending traffic on vm{i}")
                        vm_int_count_after = test_utils.get_vm_interface_counter(self.conn_obj_list[i],self.config_data['port'][i]['interface'])
                        ...
                        if stat_total + self.config_data['netperf']['delta'] >= vm_packet:
                            print(f"PASS: Minimum {vm_packet} packets transmitted and {stat_total} recorded on the path")
                        else:
