...
from common.utils.config_file_utils import get_config_dict, get_gnmi_params_simple,\
            get_interface_ipv4_dict, get_gnmi_params_hotplug,get_gnmi_phy_with_ctrl_port
        ...
        for i in range(len(self.conn_obj_list)):
            for m in range(len(self.config_data['vm'][i]['remote_ip'])):
                ...
                if m!= 0: # which is local VM ping. No ECMP path involved
                    print(f"Verify remote overlay ping {ip} on other VMs from VM{i}")
