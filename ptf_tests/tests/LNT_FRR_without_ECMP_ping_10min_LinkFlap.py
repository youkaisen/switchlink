...
class LNT_FRR_without_ECMP_ping_10min_flap(BaseTest):
    ...
    def runTest(self):
        ...
        print ("Begin to configure frr on local host")
        ...
        print (f"\nBegin to configure standard OVS on remote host {self.config_data['client_hostname']}")
        ...
        print (f"add {self.config_data['vxlan']['tep_intf']} on remote {self.config_data['client_hostname']}")
        ...
        print ("Begin to configure frr on remote {self.config_data['client_hostname']}")
        if not test_utils.vtysh_config_frr_router_interface(self.config_data['vxlan']['tep_intf'],
        ...
        while end_time>= datetime.now():
            print (f"\nThe {j} loop ping within 10 min")
            for i in range(len(self.conn_obj_list)):
                for flap in self.config_data['traffic']['flap_order']:
                    print(f"Start ping test from VM{i} to other VMs")
                    if flap=="up":
                    ...
                    for m in range(len(self.config_data['vm'][i]['remote_ip'])):
                        ...
                        if m==0: print (f"Verify local ping {ip} on VM{i}") #The 1st IP is local VM ip
                        
                        if m!= 0: #ping traffic is sent acroos physical link
                            print (f"Verify remote ping {ip} traffic from VM{i}")
                        ...
                        if flap=="down":
                            ...
                            if not test_utils.vm_to_vm_ping_drop_test(self.conn_obj_list[i], ip):
                                self.result.addFailure(self, sys.exc_info())
                                self.fail(f"FAIL: expect ping failure but success for VM{i}")
                        if m!=0:
                            print ("Record host physical port counter before sending traffic")
                            ...
                            for send_count_before,send_count_after in zip(send_count_list_before,send_count_list_after):
                                ...
                                if not stat[counter_type] >= 0:
                                    self.result.addFailure(self, sys.exc_info())
                                    self.fail(f"FAIL: Packets are not forwarded on physical port")
                            ...
                            if stat_total >= num:
                                ...
                                self.result.addFailure(self, sys.exc_info())
                                self.fail(f"FAIL: {num} packets expected but only {stat_total} transmitted")
            j+=1
