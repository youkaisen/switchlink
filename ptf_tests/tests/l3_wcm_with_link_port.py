...
class L3_WCM_Link(BaseTest):
    ...
    def runTest(self):
        ...
        print ("sending packet from TAP to Link")
        ...
        print("sending packet to link port to check if rule 1 drop")
        pkt = simple_tcp_packet(ip_dst=self.config_data['traffic']['in_pkt_header']['ip_dst'][0])
        send_packet(self, port_ids[self.config_data['traffic']['send_port'][1]], pkt)
        try:
            verify_no_packet_any(self, pkt, device_number=0, ports=[port_ids[self.config_data['traffic']['receive_port'][0]][0]])
            print(f"PASS: Verification of packets passed, packet dropped as per rule 1")
        except Exception as err:
            self.result.addFailure(self, sys.exc_info())
            print(f"FAIL: Verification of packets sent failed with exception {err}")
        ...
        print("sending packet to check if rule 4 hit")
