...
from common.utils.config_file_utils import get_config_dict, get_gnmi_params_simple, get_interface_ipv4_dict,get_gnmi_phy_with_ctrl_port,\
     get_gnmi_params_hotplug, get_interface_ipv4_dict, get_interface_ipv4_dict_hotplug
...
class LNT_FRR_with_ECMP_Hotplug_netperf(BaseTest):
    ...
    def runTest(self):
        ...
        if not ovs_utils.add_bridge_to_ovs(self.config_data['bridge'], remote=True,
            ...
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL:Failed to add bridge {self.config_data['bridge']} to \
        ...
        for namespace in self.config_data['net_namespace']: 
            ...
            if not test_utils.create_ipnetns_vm(namespace, remote=True,
                ...
                self.result.addFailure(self, sys.exc_info())
                self.fail(f"FAIL: Failed to add VM namesapce {namespace['name']} on on {self.config_data['client_hostname']}")
            ...
            if not ovs_utils.add_port_to_ovs(self.config_data['bridge'], namespace['peer_name'],
                ...
                self.result.addFailure(self, sys.exc_info())
                self.fail(f"FAIL: Failed to add port {namespace['peer_name']} to bridge {self.config_data['bridge']}")
        ...
        for i in range(len(self.config_data['remote_port'])):
            if not test_utils.vtysh_config_frr_router_interface(self.config_data['remote_port'][i],
                    ip= self.config_data['ecmp']['remote_ports_ip'][i], hostname=self.config_data['client_hostname'],
                        username=self.config_data['client_username'],password=self.config_data['client_password']):
                self.result.addFailure(self, sys.exc_info())
                self.fail(f"FAIL: Failed to config frr inteface {self.config_data['ecmp']['remote_ports_ip'][i]} on {self.config_data['client_hostname']}")
        ...
        print (f"Execute ping test to verify underlay network")
        ...
        for i in range(len(self.conn_obj_list)):
            for m in range(len(self.config_data['vm'][i]['remote_ip'])):
                ...
                for testname in self.config_data['netperf']['testname']:
                    ...
                    if m != 0:
                        print(f"Execute netperf test from VM{i} to {ip} on remote VMs")
                        print (f"Record port {vm_inteface_list[i]} counter on VM{i} before sending traffic")
                        vm_int_count_before = test_utils.get_vm_interface_counter(self.conn_obj_list[i], vm_inteface_list[i])
                        if not vm_int_count_before:
                            self.result.addFailure(self, sys.exc_info())
                            self.fail(f"FAIL: unable to get counter of {vm_inteface_list[i]}")
                    ...
                    if m != 0:
                        print (f"Record port {vm_inteface_list[i]} counter after sending traffic on VM{i}")
                        vm_int_count_after = test_utils.get_vm_interface_counter(self.conn_obj_list[i],vm_inteface_list[i]) 
                        if not vm_int_count_after:
                            self.result.addFailure(self, sys.exc_info())
                            self.fail(f"FAIL: unable to get counter of {vm_inteface_list[i]}")
                            
                        print("Record host physical counter after sending traffic")
                        ...
                        if stat_total + self.config_data['netperf']['delta']>=vm_packet:
                            ...
                            self.result.addFailure(self, sys.exc_info())
                            self.fail(f"FAIL: {vm_packet} packets expected but {stat_total} transmitted")
