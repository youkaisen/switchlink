...
class L3_Action_Selector_Create_Delete(BaseTest):
    ...
    def runTest(self):
        ...
        for group in table['del_group']:
            try:
                ovs_p4ctl.ovs_p4ctl_del_group(table['switch'],table['name'],group)
                ovs_p4ctl.ovs_p4ctl_verify_deleted_group(table['switch'],table['name'],group)
            except Exception as error:
                print("Successfully deleted the group")
                print(f"PASS: succesfully deleted group {group}")

                 
        print("Deleting members")    
        for member in table['del_member']:
            try:
                ovs_p4ctl.ovs_p4ctl_del_member(table['switch'],table['name'],member)
                ovs_p4ctl.ovs_p4ctl_verify_deleted_member(table['switch'],table['name'],member)
            except Exception as error:
                print("Successfully deleted the member")
                print(f"PASS: succesfully deleted member {member}")
