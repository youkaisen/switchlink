...
class LNT_FRR_with_ECMP_modify_BGP_ASN(BaseTest):
    ...
    def runTest(self):
        ...
        #gnmi configure interface
        if not gnmi_cli_utils.gnmi_cli_set_and_verify(self.gnmicli_params):
            self.result.addFailure(self, sys.exc_info())
            self.fail("FAIL: failed to configure gnmi cli ports")
        ...
        if not test_utils.restart_frr_service(remote=True, hostname=self.config_data['client_hostname'],
            ...
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL: failed to restart frr service on {self.config_data['client_hostname']}")
        ...
        if not gnmi_cli_utils.iplink_add_dev(self.config_data['vxlan']['tep_intf'], "dummy"):
                self.result.addFailure(self, sys.exc_info())
                self.fail(f"FAIL: Failed to add dev {self.config_data['vxlan']['tep_intf']} type dummy")
        ...
        for i in range(len(self.config_data['ecmp']['local_ports'])):
            if not test_utils.vtysh_config_frr_router_interface(self.config_data['ecmp']['local_ports'][i],
                ...
                self.result.addFailure(self, sys.exc_info())
                self.fail(f"FAIL: failed to config frr inteface {self.config_data['ecmp']['local_ports_ip'][i]} on local")
        ...
        if not ovs_utils.add_vxlan_port_to_ovs(self.config_data['bridge'],
            ...
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL: failed to add vxlan {self.config_data['vxlan']['vxlan_name'][0]} to bridge {self.config_data['bridge']}")
        ...
        for i in range(len(self.conn_obj_list)):
            ...
            if not gnmi_cli_utils.iplink_add_vlan_port(id, vlanname, self.tap_port_list[0]):
                self.result.addFailure(self, sys.exc_info())
                self.fail(f"FAIL: Failed to add vlan {vlanname} to {self.tap_port_list[0]}")
            ...
            if not ovs_utils.add_vlan_to_bridge(self.config_data['bridge'], vlanname):
                self.result.addFailure(self, sys.exc_info())
                self.fail(f"FAIL: Failed to add vlan {vlanname} to {self.config_data['bridge']}")
                    ...
                    neighbor=self.config_data['ecmp']['local_ports_ip'][i].split("/")[0], 
                        addr_family= self.config_data['bgp']['addr_family'], 
                            network=self.config_data['vxlan']['tep_ip'][1], router_id=self.config_data['vxlan']['tep_ip'][1].split("/")[0], 
                                ...
                                password=self.config_data['client_password']):
                self.result.addFailure(self, sys.exc_info())
                self.fail(f"FAIL: Failed to config Bgp on {self.config_data['client_hostname']}")
    
        print("\nSleep before sending ping traffic")
        time.sleep(5)
        print (f"Ping test for underlay network before modidying ASN-{self.config_data['bgp']['as_number'][0]}")
        ...
        print (f"Verify overlay ping before modifying ASN-{self.config_data['bgp']['as_number'][0]}")
        for i in range(len(self.conn_obj_list)):
        ...
        print (f"Begin to modify BGP ASN to {self.config_data['bgp']['as_number'][1]} on localhost")
        ...
        print (f"add new route table on localhost")
        for i in range(len(self.config_data['ecmp']['remote_ports_ip'])):                                         
            if not test_utils.vtysh_config_frr_bgp_attr(self.config_data['bgp']['as_number'][1],
                    neighbor=self.config_data['ecmp']['remote_ports_ip'][i].split("/")[0], addr_family= self.config_data['bgp']['addr_family'], 
                        network=self.config_data['vxlan']['tep_ip'][0],
                        router_id=self.config_data['vxlan']['tep_ip'][0].split("/")[0]):
        ...
        print (f"Start to modify BGP ASN on remote {self.config_data['client_hostname']}") 
        ...
        print (f"add new bgp ASN {self.config_data['bgp']['as_number'][1]} on remote")
        for i in range(len(self.config_data['ecmp']['local_ports_ip'])):                                         
            if not test_utils.vtysh_config_frr_bgp_attr(self.config_data['bgp']['as_number'][1],
                    neighbor=self.config_data['ecmp']['local_ports_ip'][i].split("/")[0], 
                        addr_family= self.config_data['bgp']['addr_family'], 
        ...
        print("\nSleep before sending ping traffic")
        time.sleep(5)  
        print (f"Ping test for underlay with new ASN {self.config_data['bgp']['as_number'][1]}")
        for ip in self.config_data['ecmp']['remote_ports_ip']:
            ping_cmd = f"ping {ip.split('/')[0]} -c 10"
            if not test_utils.local_ping(ping_cmd):
               self.result.addFailure(self, sys.exc_info())
        ...
        #overlay ping with new ASN
        print (f"Start overlay ping with new ASN {self.config_data['bgp']['as_number'][1]}")
        for i in range(len(self.conn_obj_list)):
            for ip in self.config_data['vm'][i]['remote_ip']:
                ...
                while n<=max:
                    ...
                    if not test_utils.vm_to_vm_ping_test(self.conn_obj_list[i], ip, count=self.config_data['traffic']['number_pkts'][0]):
                ...
                if stat_total>=self.config_data['traffic']["number_pkts"][0]:
                    print(f"PASS: Minimum {self.config_data['traffic']['number_pkts'][0]} tranmitted and {stat_total} recorded on the path")
                else:
                    print(f"FAIL: Minimum {self.config_data['traffic']['number_pkts'][0]} packets expected but only {stat_total} recorded")
                    self.result.addFailure(self, sys.exc_info())
