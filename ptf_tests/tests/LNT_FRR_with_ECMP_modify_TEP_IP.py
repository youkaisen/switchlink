...
class LNT_FRR_with_ECMP_modify_TEP_IP(BaseTest):
    ...
    def runTest(self):
        ...
        print (f"Begin to verify if frr is intalled and running")
        ...
        for i in range(len(self.config_data['ecmp']['remote_ports_ip'])):                                         
            if not test_utils.vtysh_config_frr_bgp_attr(self.config_data['bgp']['as_number'][0],
            ...
            if not test_utils.vtysh_config_frr_bgp_attr(self.config_data['bgp']['as_number'][0],
                    neighbor=self.config_data['ecmp']['local_ports_ip'][i].split("/")[0], 
                        addr_family= self.config_data['bgp']['addr_family'], 
                            network=self.config_data['vxlan']['tep_ip'][1], router_id=self.config_data['vxlan']['tep_ip'][1].split("/")[0], 
                                hostname=self.config_data['client_hostname'],username=self.config_data['client_username'],
                                password=self.config_data['client_password']):
                self.result.addFailure(self, sys.exc_info())
        ...
        print (f"Verify overlay ping before modifying TEP IP")
        for i in range(len(self.conn_obj_list)):
            for ip in self.config_data['vm'][i]['remote_ip']:
                print(f"Ping {ip} on other VMs from VM{i}")
                if not test_utils.vm_to_vm_ping_test(self.conn_obj_list[i], ip):
                    self.result.addFailure(self, sys.exc_info())
                    self.fail(f"FAIL: Ping test failed for VM{i}")
        
        #change TEP IP on local host
        print ("Begin to modify local TEP IP and related configuration on localhost")
        print (f"Delete old local TEP ip {self.config_data['vxlan']['tep_ip'][0]} from localhost")
        if not test_utils.vtysh_config_frr_router_interface(self.config_data['vxlan']['tep_intf'],action="no",
                ip= self.config_data['vxlan']['tep_ip'][0] ):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL: failed to config frr inteface {self.config_data['vxlan']['tep_intf']} on local")
            
        print (f"add new TEP ip {self.config_data['vxlan']['new_tep_ip'][0]} on localhost")   
        if not test_utils.vtysh_config_frr_router_interface(self.config_data['vxlan']['tep_intf'],
                ip= self.config_data['vxlan']['new_tep_ip'][0] ):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL: failed to config frr inteface {self.config_data['vxlan']['tep_intf']} on local")
            
        print (f"Delete previous bgp route table on localhost")
        if not test_utils.vtysh_config_frr_delete_bgp(self.config_data['bgp']['as_number'][0]):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL: Failed to delete bgp {self.config_data['bgp']['as_number'][0]} on localhost")
        
        print (f"add bgp attributes on localhost")
        for i in range(len(self.config_data['ecmp']['remote_ports_ip'])):                                         
            if not test_utils.vtysh_config_frr_bgp_attr(self.config_data['bgp']['as_number'][0],
                    neighbor=self.config_data['ecmp']['remote_ports_ip'][i].split("/")[0], addr_family= self.config_data['bgp']['addr_family'], 
        ...
        print (f"delete {self.config_data['vxlan']['vxlan_name'][0]} port from bridge {self.config_data['bridge']}")
        if not ovs_utils.del_port_from_ovs(self.config_data['bridge'], self.config_data['vxlan']['vxlan_name'][0]):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"Failed to delete vxlan {self.config_data['vxlan']['vxlan_name'][0]} from \
                    bridge {self.config_data['bridge']} on localhost")
            
        print (f"add {self.config_data['vxlan']['vxlan_name'][0]} port back with updated local_ip and remote ip")
        if not ovs_utils.add_vxlan_port_to_ovs(self.config_data['bridge'],
                self.config_data['vxlan']['vxlan_name'][0],self.config_data['vxlan']['new_tep_ip'][0].split('/')[0], 
                    self.config_data['vxlan']['new_tep_ip'][1].split('/')[0],self.config_data['vxlan']['dst_port'][0]):
            
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"Failed to add vxlan {self.config_data['vxlan']['vxlan_name'][0]} from \
                    bridge {self.config_data['bridge']} on localhost")
        
        #change TEP IP etc on remote host
        print ("Start to modify TEP IP and related configuration on remote {self.config_data['client_hostname']}")
        print (f"Delete old TEP ip {self.config_data['vxlan']['tep_ip'][1]} on remote {self.config_data['client_hostname']}")  
        if not test_utils.vtysh_config_frr_router_interface(self.config_data['vxlan']['tep_intf'],action="no",
                    ip= self.config_data['vxlan']['tep_ip'][1],hostname=self.config_data['client_hostname'],
                        username=self.config_data['client_username'], password=self.config_data['client_password']):
            self.result.addFailure(self, sys.exc_info())
        ...
        print (f"add new TEP ip {self.config_data['vxlan']['new_tep_ip'][1]} on remote {self.config_data['client_hostname']}")  
        if not test_utils.vtysh_config_frr_router_interface(self.config_data['vxlan']['tep_intf'],
                ip= self.config_data['vxlan']['new_tep_ip'][1],hostname=self.config_data['client_hostname'],
                    username=self.config_data['client_username'], password=self.config_data['client_password']):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"Failed to config frr inteface {self.config_data['vxlan']['tep_intf']} on {self.config_data['client_hostname']}")
            
        print (f"Delete previous route table on remote {self.config_data['client_hostname']}")
        if not test_utils.vtysh_config_frr_delete_bgp(self.config_data['bgp']['as_number'][0],
                    hostname=self.config_data['client_hostname'], 
                        username=self.config_data['client_username'],
                            password=self.config_data['client_password']):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"Failed to delete bgp {self.config_data['bgp']['as_number'][0]} on {self.config_data['client_hostname']}")
        
        print (f"add new bgp attribute on remote")
        for i in range(len(self.config_data['ecmp']['local_ports_ip'])):                                            
            if not test_utils.vtysh_config_frr_bgp_attr(self.config_data['bgp']['as_number'][0],
                    neighbor=self.config_data['ecmp']['local_ports_ip'][i].split("/")[0], 
                        addr_family= self.config_data['bgp']['addr_family'], 
        ...
        print (f"delete {self.config_data['vxlan']['vxlan_name'][0]} port from bridge {self.config_data['bridge']} on {self.config_data['client_hostname']}")
        if not ovs_utils.del_port_from_ovs(self.config_data['bridge'], 
                self.config_data['vxlan']['vxlan_name'][0], remote=True, hostname=self.config_data['client_hostname'],
                        username=self.config_data['client_username'],password=self.config_data['client_password']):
        ...
        print (f"add {self.config_data['vxlan']['vxlan_name'][0]} port back with updated local_ip and remote ip on {self.config_data['client_hostname']}")
        if not ovs_utils.add_vxlan_port_to_ovs(self.config_data['bridge'],
                self.config_data['vxlan']['vxlan_name'][0],
            ...
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL: Failed to add vxlan {self.config_data['vxlan']['vxlan_name'][0]} to \
                         bridge {self.config_data['bridge']} on on {self.config_data['client_hostname']}")
            
        print (f"Ping test for underlay network with new TEP")
        #ping remote TAP
        for ip in self.config_data['ecmp']['remote_ports_ip']:
            ping_cmd = f"ping {ip.split('/')[0]} -c 10"
            if not test_utils.local_ping(ping_cmd):
        ...
        # ping remote tep
        ping_cmd = f"ping {self.config_data['vxlan']['new_tep_ip'][1].split('/')[0]} -c 10"
        if not test_utils.local_ping(ping_cmd):
           self.result.addFailure(self, sys.exc_info())
           self.fail(f"FAIL: Ping test failed for underlay network")
        else:
        ...
        print (f"Start overlay ping with new TEP")
        for i in range(len(self.conn_obj_list)):
            for ip in self.config_data['vm'][i]['remote_ip']:
                print(f"Ping {ip} on other VMs from VM{i}")
                ...
                if stat_total>=self.config_data['traffic']["number_pkts"][0]:
                    print(f"PASS: Minimum {self.config_data['traffic']['number_pkts'][0]} packets transmitted and {stat_total} recorded on the path")
                else:
                    print(f"FAIL: Minimum {self.config_data['traffic']['number_pkts'][0]} packets transmitted but only {stat_total} recorded")
                    self.result.addFailure(self, sys.exc_info())  
